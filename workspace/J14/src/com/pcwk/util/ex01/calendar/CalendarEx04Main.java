...
public class CalendarEx04Main {
	...
	public static void main(String[] args) {
		...
		int END_DAY = 0; 			// 월의 마지막일
		
		Calendar sDay = Calendar.getInstance(); // 시작일
		Calendar eDay = Calendar.getInstance(); // 종료일
		
		// 월은 0~11까지의 값을 가지므로 1을 빼주어야 한다.
		sDay.set(year, month-1, 1); // 실제값 : 2022 2 1
		eDay.set(year, month, 1); // 실제값 : 2022 3 1
		...
		System.out.println("시작요일: "+ START_DAY_OF_WEEK); // 시작요일: 3
		
		// 월의 마지막 일
		END_DAY = eDay.get(Calendar.DATE);
		System.out.println("월의 마지막 일: "+ END_DAY); // 월의 마지막 일: 28
